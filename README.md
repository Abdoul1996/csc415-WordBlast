...
```
There is a template main with this code provided. (Don't forget to rename it to the naming conventions).  Do not modify in any way the blocks that start end end the timers.  Make sure that other than initialization code and cleanup code is between these blocks where the `TO DO`'s are.
